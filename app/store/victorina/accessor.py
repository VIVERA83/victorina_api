...
class VictorinaAccessor(BaseAccessor):
    async def get_questions_by_ids(self, ids: list[int]) -> Optional[list[QuestionModel]]:
    ...
    async def add_questions(self, questions: list[QuestionSchema]) -> list[QuestionModel]:
        async with self.app.database.session.begin().session as session:
            stmt = insert(QuestionModel).values([question.dict() for question in questions]).returning(QuestionModel)
            result = await session.execute(stmt)
